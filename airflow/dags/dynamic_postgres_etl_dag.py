...
@dag(
    dag_id="dynamic_postgres_etl_dag",
...
def dynamic_postgres_etl_dag():
    ...
    def create_dynamic_dag(**kwargs):
        try:
            ...
            dag_bag = DagBag(os.path.dirname("dags/etl_pipline_postgres.py"))
            ...
            shutil.copyfile("dags/etl_pipline_postgres.py", f"dags/{new_dag_id}.py")
            ...
            new_dag_file_content = new_dag_file_content.replace('dag_id="etl_pipline_postgres"', f'dag_id="{new_dag_id}"')
            ...
            new_dag_file_content = new_dag_file_content.replace("data['hostname']", f"'{data['hostname']}'")
            new_dag_file_content = new_dag_file_content.replace("data['portnumber']", str(data['portnumber']))
            new_dag_file_content = new_dag_file_content.replace("data['username']", f"'{data['username']}'")
            new_dag_file_content = new_dag_file_content.replace("data['password']", f"'{data['password']}'")
            new_dag_file_content = new_dag_file_content.replace("table_info = data['table_info']", "")
            new_dag_file_content = new_dag_file_content.replace("data['database_name']", f"'{data['database_name']}'")
