    ...
    def extract(ti, **kwargs):
        try:
            ...
            with engine_object.engine.connect() as connection:
                connection.execute(update_dag_run_query, dag_run_id=dag_run_id, table_id=0)
            data = kwargs['dag_run'].conf
            account = data.get('account_identifier')
            ...
            ti.xcom_push(key='table_name', value=table_info[0])
            ti.xcom_push(key='table_id', value=table_info[1])
